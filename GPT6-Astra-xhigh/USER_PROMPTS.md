# Brukerprompter for «Hvordan GPS Virker»

Denne filen inneholder samtlige brukerprompter og justeringer som er gitt i kronologisk rekkefølge for å bygge og finpusse den visuelle nettsiden. Filen inneholder **kun** brukerens egne instruksjoner (ingen AI-svar), slik at den kan brukes direkte til å gjenskape prosjektet.
...
Du mener nok mest sannsynlig GPS (en vanlig tastaturfeil på telefonen der G blir H, P blir O, og S blir S).
...
KONGSBERG & KI - Patchskill

GPS består av over 30 satellitter som kon- tinuerlig sender svært presise tidssignaler. En mottaker beregner sin egen posisjon ved å måle tiden signalene bruker...
...
Smarttelefonen som utendørs redskap - Scandinavian University Press

4. mars 2022 — En smarttelefon kan brukes til å finne sin egen posisjon nokså nøyaktig. For å få dette til, benytter man satellitter. En slik satellitt går i bane rundt jorden...

 

AI kan gjøre feil, så du bør dobbeltsjekke svarene du får

Start ved å vise 2s-prinsippet, også bygger vi ut til 3d.
...
### Prompt 2
```text
deploy to bredland.no/gps/astra (bredland.no is mounted on Y:\bredland.no)
```
...
### Prompt 3
```text
ok start, men animasjonene må være mye tydeligere og mindre rot
...
### Prompt 4
```text
sjekk og deploy igjen, nå er det ikke noe som spiller
```
...
### Prompt 5
```text
vis kulene som spheres i egne farger og miks fargene der de skjører hverandre. Det er fremdeles for mye bokser og dill rundt om. Fortellingen kan være en rolig tekst på høyre side som fader inn. Pulseringen i 2d bildet er for rotete, her også kan vi illustrere med  fargede sirkler og markere skjæringspunktene. Bilen i videoen kan stoppe innimellom også kan vi tegne på trekanter for å illustrere hva vi regner på av avstadner og vinkler og what-not. Bruk hele skjermen, legg kapittelvelger/video avspiller kontroll nederst i midten. (hel skjerm er full hd oppløsning). Legg kilde/ref til UiO i liten footer nederst til høyre. Lag en mer fancy under-overskrift, ta bort del1, del2, osv fra toppen hvis de er del av hele videoen. Alt må vi dele videoen opp i disse scenarioene.
```
...
### Prompt 6
```text
når biloen stoppes for å vise beregninger, la den stoppe der den er, ikke hopp til et gitt punkt. 3d spheres må rendres i noe transparante farger hele veien (raytrace?) og ikke med tydelig sirkel for yttergrense, skjæringslinjer og sirkler må highlightes der de mikser (ikke unionen, men akkurat skjæringslinjene - som blir sirkler, osv. Når vi legger på en satellitt til må den animeres inn og ikke bare dukke opp. Dette sskal være en visuell nytelse som også tegner og viser prinsippene. Vi kan godt bruke "papir og fargestift"-tegninger for å illustrere matematikken og ikke pinne rette datamaskinlinjer. La det ha litt sjerm. Forklaringene til illustrasjonene kan smyge seg inn og smyge seg ut i høyre kant. Alt skal forklares gjennom en rolig og personlig økt og med visuell nytelse.
```
...
### Prompt 7
```text
spherene må være mye tydeligere og skjæringen illustreres med sterkere farge, men ikke med stiplet linje.
overgangen mellom temaene må gjøres pene og smarte.
vi kan bruke mer tegning på skissene våre
i 3d trenger vi ikke vise jorden og raskt forflyttende punkt, men vi kan vise satelittspheres og skjæringne også plassere jorden inn også bilen, men da antar jeg vi må zoome inn på en del av den siden spheres til satelittene ikke dekker over hele kloden. vi må også vise hvilke skjæringer/punkter vi enkelt kan forkaste og hvorfor.
```
...
doppler må få matematikken tenget på animasjonen
vi kan godt bruke strek som ser mer hjemmetenget (blyant, fargestifter) enn de data-genererte linjene som er der nå
...
```
Ikke se på Gemini eller Fables kataloger og svar.